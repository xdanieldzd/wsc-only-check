...
/*
    ...
    WSCERR_ENABLE_BACKGROUND (optional):         Enables background graphics support; requires the information inside the respective #ifdef (i.e. WSCERR_FONT_FILENAME, WSCERR_BACKGROUND_TILES, etc.)
    WSCERR_ENABLE_BACKGROUND_SCROLL (optional):  Enables scrolling of background; requires background graphics to be enabled; configure speed via WSCERR_BACKGROUND_SCROLL_SPEED
    WSCERR_ENABLE_TEXT_BLINK (optional):         Enables error message text blinking; total loop duration is ~3 seconds, for 1/5 of which the text is not visible
